...
**YOU USE THIS SOFTWARE AT YOUR OWN RISK. THE AUTHOR (BONITO) IS NOT RESPONSIBLE FOR:**
...
- Financial losses from any cause
...
- If you deploy to Ethereum mainnet (instead of testnet): **real money will be at stake**
- Gas fees will cost real ETH
- Smart contract bugs = permanent financial loss
- No recovery, no refunds, no support
- **This is extremely risky**

❌ **Testnet Deployment:**
- Even on testnet, the code could have bugs
- Smart contracts are experimental and unaudited
- Use at your own risk

### Your Full Responsibility

You accept **100% of all risks** by using this software:

1. **Setup & Configuration** — You're responsible for proper configuration
2. **Security** — You protect all credentials, keys, and API tokens
3. **Funds** — You own any crypto assets used (real or testnet)
4. **Execution** — You understand what this code does before running it
5. **Outcomes** — You accept all results, good or bad
6. **Compliance** — You follow all local laws and regulations
...
│
├── 🚀 QUICK START
│   ├── start_all.sh                 # One-command startup (runs everything)
│   ├── stop_all.sh                  # Shutdown all services
│   └── .env.example                 # Configuration template (copy to .env)
│
├── 🐋 CORE SYSTEM (Whale Detection)
│   ├── whale_tracker_integrated.py  # Scans Ethereum, creates whale_data.json
│   ├── whale_profiler.py            # Classifies whale types
│   ├── whale_watch_config.py        # Configuration management
│   └── alert_system.py              # Alert generation
│
├── 🔌 REST API
│   ├── whale_api_enhanced.py        # FastAPI endpoints + smart contract integration
│   └── deploy_contract.py           # Deploy script for smart contract
│
├── 🤖 SMART CONTRACT
│   └── WhalePredictionMarket.sol     # Solidity contract (Base Sepolia)
│
├── 📊 DASHBOARDS
│   ├── dashboard-enhanced.html       # Main dashboard (Predictions + Whales)
│   └── dashboard-simple.html         # Simple whale tracker
│
├── 📚 DOCUMENTATION
│   ├── README.md                     # This file (complete guide)
│   ├── DEPLOYMENT_GUIDE.md           # How to deploy smart contract
│   ├── QUICKSTART_PREDICTION_MARKET.md # Quick start for predictions
│   ├── API-AGENT-GUIDE.md            # API integration examples
│   ├── FEATURES-EXTRAS.md            # Advanced features
│   └── .gitignore                    # Git exclusions (protects .env)
│
└── ⚙️ UTILITIES
    ├── requirements.txt              # Python dependencies
    ├── whale_data.json               # Live whale data (generated by tracker)
    └── logs/                         # Log files (created by start_all.sh)
```
